...
lemma exp_bernoulli_exp {p : ℝ} (m : p ∈ Icc 0 1) (t : ℝ) :
    (bernoulli p).exp (fun x ↦ (t * bif x then 1 else 0).exp) = 1 - p + p * t.exp := by
  simp only [exp_bernoulli _ m, cond_false, mul_zero, Real.exp_zero, smul_eq_mul, mul_one,
    cond_true]
...
lemma L_d0 : HasDerivAt (fun t ↦ p * t.exp) (p * t.exp) t := (Real.hasDerivAt_exp _).const_mul _
lemma L_d1 : HasDerivAt (fun t ↦ 1 - p + p * t.exp) (p * t.exp) t := L_d0.const_add _
lemma dL (m : p ∈ Icc 0 1) : HasDerivAt (L p) (p * t.exp / (1 - p + p * t.exp)) t :=
  L_d1.log (ne_of_gt (L_pos m))
lemma ddL (m : p ∈ Icc 0 1) :
    HasDerivAt (fun t ↦ deriv (L p) t) ((p * t.exp * (1 - p:)) / (1 - p + p * t.exp)^2) t := by
  have h : HasDerivAt (fun t ↦ deriv (L p) t)
      ((p * t.exp * ((1 - p:) + p * t.exp) -
...
lemma hoeffdings_lemma {p : ℝ} (m : p ∈ Icc 0 1) {t : ℝ} (t0 : 0 ≤ t) :
    (bernoulli p).exp (fun x ↦ (t * bif x then 1 else 0).exp) ≤ (t*p + t^2/8).exp := by
...
lemma hoeffdings_lemma' (f : Prob Bool) {t : ℝ} (t0 : 0 ≤ t) :
    f.exp (fun x ↦ (t * bif x then 1 else 0).exp) ≤ (t * f.prob true + t^2/8).exp := by
...
lemma chernoff_le_count (f : Prob Bool) (n : ℕ) {t : ℝ} (t0 : 0 ≤ t) :
    (count f n).pr (fun x ↦ x ≤ n * f.prob true - t) ≤ (-2 * t^2 / n).exp := by
...
lemma chernoff_count_abs_le (f : Prob Bool) (n : ℕ) {t : ℝ} (t0 : 0 ≤ t) :
    (count f n).pr (fun x ↦ t ≤ |x - n * f.prob true|) ≤ (2:ℝ) * ((-2:ℝ) * t^2 / n).exp := by
...
lemma chernoff_estimate_abs_le (f : Prob Bool) (n : ℕ) {t : ℝ} (t0 : 0 ≤ t) :
    (estimate f n).pr (fun x ↦ t ≤ |x - f.prob true|) ≤ (2:ℝ) * ((-2:ℝ) * n * t^2).exp := by
  by_cases n0 : (n : ℝ) = 0
